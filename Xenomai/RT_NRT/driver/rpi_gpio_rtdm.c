/*
 * GPIO RTDM driver example for Raspberry Pi (rt/nrt)
 */
...
// default GPIO pins
static int gpio_rt = 25;
static int gpio_nrt = 24;

module_param(debug, int, 0644);
module_param(gpio_rt, int, 0644);
module_param(gpio_nrt, int, 0644);
...
static ssize_t rpi_gpio_ioctl_rt(struct rtdm_dev_context* context, rtdm_user_info_t* user_info, unsigned int request, void __user* arg)
{
  ...
  // Set or Clear
  if (request == 0)
    GPIO_SET(ctx->gpio_addr) = (1 << ctx->gpio_rt);
  else
    GPIO_CLR(ctx->gpio_addr) = (1 << ctx->gpio_rt);
